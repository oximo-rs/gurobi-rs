...
//! be found [here](https://github.com/oximo-rs/gurobi-rs/tree/master/crates/gurobi-rs/examples).
//!
...
//! assert_eq!(model.get_obj_attr(attr::VarName, &x1).unwrap_err(), gurobi_rs::Error::ModelObjectPending);
...
cfg_if::cfg_if! {
    if #[cfg(not(any(feature = "gurobi13", feature = "gurobi12", feature = "gurobi11", feature = "gurobi10")))] {
        compile_error!(
            concat!(
                ...
                "setting gurobi13 and gurobi10 is equivalent to only setting gurobi13."
            )
        );
    } else {
        mod lib_impl;
