...
impl Model {
    ...
    /// let x = add_ctsvar!(m);
    ///
    /// assert_eq!(m.try_clone().err().unwrap(), gurobi_rs::Error::ModelUpdateNeeded);
    ///
    /// m.update();
    ...
    /// # use gurobi_rs::prelude::*;
    ///
    /// fn compute_iis_constraints(m: &mut Model) -> gurobi_rs::Result<Vec<Constr>> {
    ///    m.compute_iis()?;
    ...
    /// assert!(matches!(r, Err(gurobi_rs::Error::AlgebraicError(_))));
    /// # Ok::<(), gurobi_rs::Error>(())
    /// ```
    ///
    ///
    ...
    /// assert_eq!(m.get_constr_by_name("constraint").unwrap_err(), gurobi_rs::Error::ModelUpdateNeeded);
    /// m.update()?;
...
mod tests {
    use super::*;
    extern crate self as gurobi_rs;
