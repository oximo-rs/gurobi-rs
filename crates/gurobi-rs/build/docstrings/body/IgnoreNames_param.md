...
In addition, the parameter affects the behavior of the write functions (e.g. GRBwrite in C, or Model.write in Python).
If `IgnoreNames` is set to 1, Gurobi uses default names when writing the file. This can be useful if you have a model
with names and want to write the model, the attributes, a MIP start file, or other information to disk without including
