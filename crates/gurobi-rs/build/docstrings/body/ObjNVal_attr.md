This attribute is used to query the objective value obtained for objective \(n\) by the \(k\) -th solution stored in the
pool of feasible solutions found so far for the problem. You set \(n\) using the ObjNumber parameter, while you set
\(k\) using the SolutionNumber parameter.
