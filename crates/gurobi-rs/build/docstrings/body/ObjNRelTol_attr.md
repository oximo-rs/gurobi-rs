...
allowed to degrade this value by at most `ObjNRelTol` * \(|z|\) .

Objective degradations are handled differently for multi-objective LP models. The allowable degradation is controlled by
the `ObjNAbsTol` parameter and `ObjNRelTol` is ignored.
