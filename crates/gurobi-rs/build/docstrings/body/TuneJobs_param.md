...
causes the tuning tool to use a static set of up to n workers in parallel. Such workers are kept for the whole tuning
run. Note that this parameter can be combined with `TuneDynamicJobs` to get a static set of workers and a dynamic set of
workers for distributed tuning. You can use the `WorkerPool` parameter to provide a distributed worker cluster.
