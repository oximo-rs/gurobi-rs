...
fixings are decided using variable reduced costs. The value of the `ObjNAbsTol` attribute indicates the amount by which
a fixed variable’s reduced cost is allowed to violate dual feasibility. Relaxing this tolerance has a similar effect on
the value of the objective to relaxing OptimalityTol , so we recommend against values larger than 1e-2. The value of the
related `ObjNRelTol` attribute is ignored.
