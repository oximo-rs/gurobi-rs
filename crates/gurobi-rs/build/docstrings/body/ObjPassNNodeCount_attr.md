This attribute is used to query the number of explored nodes for optimization pass \(p\) of a multi-objective solve. You
set \(p\) using the ObjPassNumber parameter. If that parameter is set to -1 (its default value), the optimization pass
