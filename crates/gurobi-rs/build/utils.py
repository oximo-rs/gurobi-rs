...
def _clean_documentation(s: str) -> list:
    paragraphs = map(
        lambda x: remove_newlines(x.strip()), filter(bool, s.split("\n\n"))
    )
    paragraphs = [p for p in paragraphs if not any(r.fullmatch(p) for r in DOC_REMOVE)]

    return paragraphs


_DTYPES = {"double": "dbl", "string": "str", "int": "int", "char": "chr"}


async def fetch_parameter_data(
    session: aiohttp.ClientSession, name: str, path: str
) -> dict:
    ...
    ty = (
        table.find(string="Type:", recursive=True)
        .parent.parent.find_next_sibling("td")
        .text
    )
    data["ty"] = ty
    ...
    data["default"] = (
        table.find(string="Default value:", recursive=True)
        .parent.parent.find_next_sibling("td")
        .text
    )
    if ty == "int" or ty == "double":
        data["min"] = (
            table.find(string="Minimum value:", recursive=True)
            .parent.parent.find_next_sibling("td")
            .text
        )
        data["max"] = (
            table.find(string="Maximum value:", recursive=True)
            .parent.parent.find_next_sibling("td")
            .text
        )
...
async def fetch_attribute_data(
    ...
    mod = (
        table.find(string="Modifiable:", recursive=True)
        .parent.parent.find_next_sibling("td")
        .text
    )
    object_ty = (
        table.parent.find(class_="navigation")
